...
const SupportForm = ({ isLightTheme }) => {
    const themeClass = isLightTheme ? 'light-theme' : 'dark-theme';
    const [form] = Form.useForm();

    const onFinish = async (values) => {
        try {
            // URL вашого бек-енд сервера
            const serverUrl = `${process.env.REACT_APP_API_URL}/api/send-support-request`;

            const requestData = {
                ...values,
                subject: 'Запит з підтримки',
            };
                    ...
                    >
                        ...
                        <Form.Item
                            ...
                            rules={[
                                ...
                                { pattern: /^\+?[0-9()\s-]+$/, message: "Будь ласка, введіть коректний номер телефону" }
                            ]}
                        >
